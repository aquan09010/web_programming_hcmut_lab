...
const create = () => {
  ...
  <tbody id="table-body">
  <tr class="border-y bg-white even:bg-gray-100 hover:bg-gray-200">
  <td class="px-4">1</td>
  <td class="px-4 py-3">Yuta Okkotsu</td>
</tr>
  </tbody>
</table>`;
...
document.querySelector(".del-row").onclick = () => {
  ...
  no > 0 && no < numOfRow
    ? deleteRow(no)
    : alert(
        `You can delete from row 1 to row ${numOfRow - 1}, default row is 0!`
      );
};
...
const addRow = () => {
  ...
  const newRow = `<tr class="border-y bg-white even:bg-gray-100 hover:bg-gray-200">${cell}</tr>`;
